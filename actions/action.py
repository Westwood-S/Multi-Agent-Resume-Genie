...
class SkillMatch(Action):
    ...
    **Goal**:
    Extract a structured breakdown of critical job requirements and insights to help applicants align their profiles effectively.
    ...
    **Format**:
    Please provide your response in a structured JSON format with all the categories in **Instructions**.
...
class ProfileEnhance(Action):
    ...
    **Goal**:
    Refine the resume to highlight the applicant's most relevant skills, experiences, and qualities, ensuring alignment with the job's requirements to make a lasting impression.
...
class ResumePolish(Action):
    ...
    **Goal**:
    Refine the resume to emphasize the applicant's core skills, aligned experiences, educational background, and traits, making it resonate strongly with the job requirements.
...
class InterviewCoach(Action):
    ...
    **Goal**:
    Create a preparation guide that includes targeted interview questions, suggested answers, and main talking points, allowing the candidate to highlight their most relevant skills, experiences, and accomplishments effectively.
